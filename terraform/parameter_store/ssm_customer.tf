locals {
    customer_production_parameters = {
        ...
        "Postgres/ConnectionString" = { type = "SecureString", value = "Host=postgres-service;Port=5432;Database=${var.postgres-default-db};Username=customer_user;Password=customer_${var.postgres-default-password}" }
    }
}

resource "aws_ssm_parameter" "customer_docker_params" {
    for_each = local.customer_production_parameters
    name        = "/HotelUp.Customer/Production/${each.key}"
